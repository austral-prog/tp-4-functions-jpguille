...
def final_price(price, quantity, discount_pct, tax_pct):
    ...
    price_with_discount = apply_discount(subtotal, discount_pct)
    total = apply_tax(price_with_discount, tax_pct)
...
def best_deal(price_a, qty_a, disc_a, price_b, qty_b, disc_b, tax_pct):
    ...
    """
    if final_price(price_b, qty_b, disc_b, tax_pct) < final_price(price_a, qty_a, disc_a, tax_pct):
        return "B"
    else:
        return "A"
